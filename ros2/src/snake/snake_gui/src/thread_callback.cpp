...
ThreadCallback::ThreadCallback(QObject *parent) : QThread(parent) {
}

ThreadCallback::~ThreadCallback() = default;

void ThreadCallback::run() {
//    rclcpp::spin(node_);
...
void ThreadCallback::setNode(std::shared_ptr<snake::SnakeGUINode> node) {
    node_ = std::move(node);
}
