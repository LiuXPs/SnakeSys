...
def generate_launch_description():
    ...
    settings_file_path = os.path.join(slam_pkg_dir, 'config', 'mono_imu_d455.yaml')
    ...
    slam_node = Node(
        package='snake_slam',
        executable='snake_slam_mono_imu',
        name='snake_slam_mono_imu',
        namespace='snake',
        output="screen",
        remappings=[
            ('/imu', '/camera/imu'),
            ('/camera/mono/image_raw', '/camera/infra1/image_rect_raw'),
            ('/camera/mono/image_raw/compressed', '/camera/infra1/image_rect_raw/compressed'),
        ],
        parameters=[{
