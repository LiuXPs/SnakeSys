...
int main(int argc, char **argv) {
    const std::string node_name = "snake_slam_stereo_imu";

    rclcpp::init(argc, argv);
    rclcpp::executors::MultiThreadedExecutor exe;
    auto node = std::make_shared<SnakeSLAMNode>(node_name,
                                                SnakeSLAMNode::SLAMSystem::ORB_SLAM3,
                                                SnakeSLAMNode::SLAMType::STEREO_IMU);
    exe.add_node(node);
    exe.spin();
